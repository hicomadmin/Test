...
ICore.Page {
    ...
    IControls.ListViewC {
            ...
            onClicked: {
                application.changePage('music',{
                                    properties: { curTrack:trackName,
                                                  list:artalbmutracklist, modelData:listModel
                                           },needUpdate: true
                               } )
            }
        }
        ...
        listView.paginationLoaderDelegate: UControls.PaginationLoaderDelegate {
            function loadPageAsync(page , cb) {
                ...
                usbctl.artistalbumtracklistChanged.connect(function handler(artistalbumtracklist){
                    usbctl.artistalbumtracklistChanged.disconnect(handler);
                    cb(null, artistalbumtracklist) ;
                });
                var data = ({'artistname':artistName , 'albumname': albName ,'page':page})
                usbctl.requestLoadData("getartistalbumtracklist", data);
            }
        }
    }
    ...
    Item {
        ...
        UControls.RatioText {
            ...
            denText: artalbmutracklist ?  artalbmutracklist.model.count : 0
            numText: artalbmutracklist  ?  artalbmutracklist.listView.listView.currentIndex+1 : 0
        }
    }
}
